...
impl RollingFileAppenderComponent {
    pub fn new_policy() -> Box<CompoundPolicy> {
        ...
        let roller: Box<FixedWindowRoller> = Box::new(
            FixedWindowRoller::builder()
                .base(*ROLLER_BASE_START)
                .build(&ROLLER_FILEPATH_PATTERN, *ROLLER_MAX_COUNT)
                .unwrap(),
        );
        let compound_policy: Box<CompoundPolicy> =
        ...
        let Global_logs_config: Config = Config::builder()
            ...
            .logger(
                Logger::builder()
                    ...
                    .additive(true)
                    .build("All-Logs", LevelFilter::Info),
            )
            .logger(
                Logger::builder()
            ...
            .logger(
                Logger::builder()
                    .appender("console_appender")
                    .additive(true)
                    .build("inventyv_datalayer_lib", LevelFilter::Off),
            )
            .logger(
                Logger::builder()
                    .appender("console_appender")
                    .additive(true)
                    .build("couchbase", LevelFilter::Error),
            )
            .build(
                Root::builder()
